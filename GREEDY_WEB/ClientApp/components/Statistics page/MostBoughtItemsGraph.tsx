...
export class MostBoughtItemsGraph extends React.Component<Props, State> {
    state = {
        graphData: [],
        showGraphs: false,
        time: 3600
    ...
    readGraphData(/*time*/) {
        axios.put(Constants.httpRequestBasePath + 'api/MostBoughtItemsGraphData', this.state.time,
            {
                headers: {
                ...
                {this.state.showGraphs ?
                    <div className="row">
                        <div className="col-md-6 dataGraph">
                            <h3 className="graphTitles h3">Top 3 most bought items</h3>
                            <BarChart width={450} height={150} data={this.state.graphData}>
                                ...
                                <YAxis />
                                <Legend />
                                <Bar name="Count" type='monotone' dot={false} dataKey='Count' stroke='#8884d8' strokeWidth={2} fill='#ffbfbf'/>
                            </BarChart>
                        </div>
                    </div> :
