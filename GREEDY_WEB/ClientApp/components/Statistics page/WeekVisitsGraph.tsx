...
export class WeekVisitsGraph extends React.Component<Props, State> {
    state = {
        graphData: [],
        showGraphs: false,
        time: 3600
    ...
    readGraphData(/*time*/) {
        axios.put(Constants.httpRequestBasePath + 'api/WeekVisitsGraphData', this.state.time,
            {
                headers: {
                ...
                {this.state.showGraphs ?
                    <div className="row">
                        <div className="col-md-6 dataGraph">
                            <h3 className="graphTitles h3">Week visits</h3>
                            <LineChart width={450} height={150} data={this.state.graphData}>
                                {
                                    /*this.state.time < 3600 * 12 ?
                                        <XAxis dataKey="Time" /> :*/
                                        <XAxis dataKey="WeekDayString" />
                                }
                                <Tooltip />
                                <YAxis />
                                <Legend />
                                <Line name="Visits" type='monotone' dot={false} dataKey='VisitsCount' stroke='#8884d8' strokeWidth={2} />
                            </LineChart>
                        </div>
                    </div> :
